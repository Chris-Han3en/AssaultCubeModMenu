...
namespace AssaultCubeHacks
{
    class MainFunctions
    {
        ...
        public static IntPtr FindDMAAddy(IntPtr hProc, IntPtr ptr, int[] offsetse)
        {
            ...
            var thing = ptr.ToString("X");//this exists due to a problem where it would randomly add 3 charecters at the start of each memory address. i have no idea why
            if (thing.Length > 8)
            {
                thing = thing.Remove(0, 3);
                ptr = new IntPtr(Convert.ToInt32(thing, 16));
            }
            return ptr;
        }
